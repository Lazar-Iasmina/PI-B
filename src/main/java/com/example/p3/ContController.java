...
public class ContController {
    ...
    String ALeg;
    public void displayDestinatie(String email) throws FileNotFoundException {
        ...
        nume.setText(n+" "+p);
        System.out.println("###"+uname1+x);
        List<String> l=new ArrayList<>();
        for(ZborRezervat i:zr){
        ...
        listViewCont.getItems().addAll(l);
        listViewCont.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observableValue, String s, String t1) {
                ALeg=listViewCont.getSelectionModel().getSelectedItem();
                zborSelectat.setText(ALeg);
            }
        });
    ...
    public void switchtoScenaZbor(ActionEvent event) throws IOException {
        String a=zborSelectat.getText();
        ...
        scene2Controller.displayDestinatie(a);
        stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
        scene = new Scene(root);
        stage.setScene(scene);
        stage.show();;
