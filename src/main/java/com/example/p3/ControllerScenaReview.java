...
public class ControllerScenaReview implements Initializable {
    private Stage stage;
    private Scene scene;
    private Parent root;
    @FXML
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        dest.getItems().addAll(listaDest);
        dest.setOnAction(this::getFood);
        ...
        l.add(4);
        l.add(5);
        stele.getItems().addAll(l);
        stele.setOnAction(this::getStea);


    }

    public void getFood(ActionEvent e) {
        String myFood = dest.getValue();
        select.setText(myFood);
    }

    public void getStea(ActionEvent e) {
        String myFood = String.valueOf(stele.getValue());
        select1.setText(myFood);
    }

    public void trimitereReview(ActionEvent e) throws IOException {
        String dest = new String();
        String stea = new String();
        String rev = new String();
        dest = select.getText();
        stea = select1.getText();
        rev = review.getText();
        ...
        max = max + 1;
        String g = select1.getText();
        List<Destinatie> d = new ArrayList<>();
        d = citireDestinatii();
        int x = 0;
        for (Destinatie i : d) {
            if (i.getNume().equals(select.getText())) {
                x = i.getId();
            }
        }
        Review r = new Review(max, x, review.getText(), Integer.parseInt(g));
        rr.add(r);
        updateReviewuri(rr);
