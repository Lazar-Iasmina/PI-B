...
public class ControllerChat {
    ...
    @FXML
    Label idz;
    public void displayDestinatie(String idzz){
        List<Mesaj>m=new ArrayList<>();
        m=citireMesaje();
        List<ZborRezervat> zb=new ArrayList<>();
        zb=citireZboruriRezervate();
        int x=Integer.parseInt(idzz);
        ...
        mesaje.getItems().addAll(l);
        idz.setText(String.valueOf(idzz));



    }
    public void send(ActionEvent e){
        String ms=mesaj.getText();
        mesaj.setText("");
        ...
        max=max+1;
        Mesaj m=new Mesaj(max,Integer.parseInt(idz.getText()),"Client : "+ms,2);
        msg.add(m);
        updateMesaje(msg);
        mesaje.getItems().add("Client : "+ ms);
